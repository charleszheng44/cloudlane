...
for test_name in crypto_test storage_test mpris_test loginflow_test theme_test; do
  test_dir="build/${test_name//_/-}"
  mkdir -p "$test_dir"
  qmake6 -o "$test_dir/Makefile" "tests/$test_name.pro"
  make -C "$test_dir" -j"${CLOUDLANE_JOBS:-${YUNJIAN_JOBS:-4}}" > "$test_dir/build.log" 2>&1
done
./build/crypto-test/crypto-test tests/transport-vectors.json
./build/loginflow-test/loginflow-test
./build/theme-test/theme-test
...
if [[ "${CLOUDLANE_NATIVE_TESTS:-${YUNJIAN_NATIVE_TESTS:-0}}" == 1 ]]; then
  mkdir -p build/native-test
  qmake6 -o build/native-test/Makefile tests/native_test.pro
  make -C build/native-test -j"${CLOUDLANE_JOBS:-${YUNJIAN_JOBS:-4}}" > build/native-test/build.log 2>&1
  ...
  YUNJIAN_TEST_MEDIA="$repo_dir/build/test-media.mp4" YUNJIAN_LIVE_TESTS="${CLOUDLANE_LIVE_TESTS:-${YUNJIAN_LIVE_TESTS:-0}}" ./build/native-test/native-test
fi
